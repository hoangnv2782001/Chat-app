...
export const TextMessage = ({ message, menu, id }) => {
  ...
  return (
    <Stack
      direction="row"
      justifyContent={message.sender.id === id ? "end" : "start"}
    >
      <Tooltip key={id} placement="right" title={fDateTime(message?.time)}>
        <Box
          ...
          sx={{
            ...
            width: "max-content",
            backgroundColor:
              message.sender.id !== id ? "#F0F0F0" : theme.palette.primary.main,
          }}
        >
          <Typography
            variant="body2"
            color={message.sender.id !== id ? theme.palette.text : "#fff"}
          >
            {message.content}
          </Typography>
        </Box>
            ...
            <Stack spacing={2}>
              ...
              >
                ...
                <Typography
                  ...
                  sx={{
                    color:
                      message.sender.id !== id ? theme.palette.text : "#000",
                  }}
                >
                  {name}
                </Typography>
